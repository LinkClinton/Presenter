...
namespace Presenter
{
    ...
    public static partial class GraphicsPipeline
    {
        ...
        public static void Reset(GraphicsPipelineState GraphicsPipelineState)
        {
            ...
            Engine.ID3D11DeviceContext.VertexShader.Set(graphicsPipelineState.VertexShader.ID3D11VertexShader);
            Engine.ID3D11DeviceContext.PixelShader.Set(graphicsPipelineState.PixelShader.ID3D11PixelShader);

            Engine.ID3D11DeviceContext.InputAssembler.InputLayout = graphicsPipelineState.ID3D11InputLayout;
            Engine.ID3D11DeviceContext.OutputMerger.DepthStencilState = graphicsPipelineState.ID3D11DepthStencilState;
            Engine.ID3D11DeviceContext.OutputMerger.BlendState = graphicsPipelineState.ID3D11BlendState;
            Engine.ID3D11DeviceContext.Rasterizer.State = graphicsPipelineState.ID3D11RasterizerState;
        }
    }
}
